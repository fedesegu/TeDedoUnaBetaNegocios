...
//         res.render("home",  { uid: req.user.id, user: req.user, name: req.user.name, email : req.user.email, nextPage: nextPage,  prevPage: prevPage, style: "product" } );
//     } catch (error) {
//         // console.error(error);
...
// router.get("/users", passport.authenticate("current", {session:false}), async (req, res) => {
...
//             const [products] = await pool.query("SELECT * FROM products WHERE id = ?", [pid]);
...
//             const clonedProduct = { ...productFound };
